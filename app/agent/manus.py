                ...
                for line in lines:
                    ...
                    if line.startswith("## Phase"):
                        try:
                                ...
                                if phase_num <= len(self.current_plan["phases"]):
                                    if phase_num <= self.current_phase + 1:
                                        if "[PENDING]" in line:
                                            ...
                                            "[CURRENT]" in line
                                            and phase_num < self.current_phase + 1
                                        ):
                                            line = line.replace(
                                                "[CURRENT]", "[COMPLETE]"
                                            )
                            ...
                            logger.warning(
                                f"Could not parse phase number from line: {line}, error: {e}"
                            )
...
"""
    ...
    async def get_current_phase_guidance(self) -> str:
        ...
        if not self.current_plan or self.current_phase >= len(
        ...
        return self.reasoning_framework.get_next_step_guidance(
            self.current_step + 1, len(current_phase_info["steps"]), current_phase_info
        )
    ...
    async def advance_step(self):
        ...
        if self.current_step >= len(current_phase_info["steps"]) - 1:
            ...
            self.current_step += 1
            logger.info(
                f"Advanced to step {self.current_step + 1} in phase {self.current_phase + 1}"
            )
    ...
    async def force_create_todo_file(self, plan: Dict) -> str:
        ...
        for i, phase in enumerate(plan["phases"]):
            ...
            todo_content += f"**🎯 Success Criteria:** {phase['success_criteria']}\n\n"
            todo_content += (
                f"**🛠️ Tools Required:** {', '.join(phase['tools_needed'])}\n\n"
            )
        ...
        try:
            ...
            if str_editor:
                ...
                result = await str_editor.execute(
                    command="create", path=self.todo_file_path, file_text=todo_content
                )
                logger.info(
                    f"EMERGENCY: Todo list created using str_replace_editor: {self.todo_file_path}"
                )
            else:
                ...
                logger.info(
                    f"EMERGENCY: Todo list created using new str_replace_editor instance: {self.todo_file_path}"
                )

            return todo_content
        except Exception as e:
            logger.error(
                f"EMERGENCY: Failed to create todo list with str_replace_editor: {e}"
            )
            ...
            try:
                with open(self.todo_file_path, "w", encoding="utf-8") as f:
                    f.write(todo_content)
                logger.info(
                    f"EMERGENCY: Todo list saved with fallback method to {self.todo_file_path}"
                )
            except Exception as e2:
                logger.error(
                    f"EMERGENCY: Failed to save todo list even with fallback: {e2}"
                )
            return todo_content

    async def create_analysis_file(
        self, content: str, filename: str = "analysis.md"
    ) -> str:
        ...
        try:
            ...
            result = await str_editor.execute(
                command="create", path=analysis_path, file_text=content
            )
            logger.info(
                f"EMERGENCY: Analysis file created using str_replace_editor: {analysis_path}"
            )
            return content
        except Exception as e:
    ...
    async def create_summary_file(
        self, content: str, filename: str = "summary.md"
    ) -> str:
        ...
        try:
            ...
            result = await str_editor.execute(
                command="create", path=summary_path, file_text=content
            )
            logger.info(
                f"EMERGENCY: Summary file created using str_replace_editor: {summary_path}"
            )
            return content
        except Exception as e:
    ...
    async def step(self) -> str:
        ...
        if self.current_plan is None or not self.current_plan:
            logger.info(
                "EMERGENCY: Forcing plan creation due to None/empty current_plan"
            )
...
"""
        ...
        if hasattr(self, "memory_system") and self.current_plan:
            ...
            logger.info(
                f"🎓 APPLYING LEARNED STRATEGY: Confidence {learned_strategy['confidence_score']:.2f}"
            )

            # Integrate learning insights into guidance
            if learned_strategy["optimization_elements"]:
                guidance += f"\n\n🎓 LEARNED OPTIMIZATIONS TO APPLY:\n"
                for optimization in learned_strategy["optimization_elements"][
                    :3
                ]:  # Top 3
            ...
            if learned_strategy["quality_enhancements"]:
                ...
                for enhancement in learned_strategy["quality_enhancements"][
                    :3
                ]:  # Top 3
...
"""
        ...
        try:
            ...
            if hasattr(self, "memory_system") and self.current_plan:
                task_type = self._categorize_task(self.current_plan.get("goal", ""))
                approach_used = (
                    f"phase_{self.current_phase + 1}_step_{self.current_step + 1}"
                )
                context = {
                    "phase": self.current_phase,
                    "step": self.current_step,
