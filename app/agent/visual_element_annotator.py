...
class VisualElementAnnotator:
    ...
    async def find_element_by_description(self, element_description: str) -> Dict:
        ...
        if result["success"]:
            ...
            if element_number is not None:
                ...
                browser_index = self._map_to_browser_index(element_number, result["element_map"])
    ...
    async def _get_elements_and_screenshot(self) -> Dict:
        ...
        try:
            ...
            if hasattr(result, 'error') and result.error:
                return {"success": False, "error": f"Browser extraction failed: {result.error}"}
    ...
    async def _annotate_screenshot_with_elements(self, screenshot_base64: str, elements: List[Dict]) -> Dict:
        ...
        try:
            ...
            for i, element in enumerate(elements):
                ...
                bounds = element.get("bounds", self._estimate_element_bounds(i, len(elements), image.size))
                ...
                draw.rectangle([label_x, label_y, label_x + label_width + 6, label_y + label_height + 4], 
                             fill="red")
                
                # Draw label text
                draw.text((label_x + 3, label_y + 2), label, fill="white", font=font)
                
                logger.info(f"📍 Annotated element {element_number}: {element.get('type', 'unknown')} at ({x1}, {y1})")
    ...
    async def _analyze_annotated_screenshot(self, annotated_screenshot: str, elements: List[Dict], goal: str) -> str:
        """Analyze annotated screenshot with Llama 3.2 Vision"""
        logger.info("🧠 Analyzing annotated screenshot with Llama 3.2 Vision")
        ...
        try:
            ...
            element_guide = "\\n".join([
                f"Element {i+1}: {elem.get('type', 'unknown')} - {elem.get('text', 'no text')[:50]}"
                for i, elem in enumerate(elements)
            ])
            
            # Prepare vision prompt with element context
            vision_prompt = f"""
            {goal}
            
            This screenshot shows a webpage with interactive elements marked by numbered red boxes.
            
            Element Reference:
            {element_guide}
            
            Please analyze the visual elements and provide specific element numbers in your response.
            """
            
            # Prepare image data
            image_url = f"data:image/png;base64,{annotated_screenshot}"
    ...
    def _extract_elements_from_content(self, browser_result) -> List[Dict]:
        ...
        try:
            ...
            mock_elements = [
                {"index": 0, "type": "input", "text": "search", "bounds": {"x": 300, "y": 200, "width": 400, "height": 40}},
                {"index": 1, "type": "button", "text": "Google Search", "bounds": {"x": 350, "y": 250, "width": 120, "height": 35}},
                {"index": 2, "type": "button", "text": "I'm Feeling Lucky", "bounds": {"x": 480, "y": 250, "width": 140, "height": 35}},
                {"index": 3, "type": "link", "text": "Gmail", "bounds": {"x": 600, "y": 50, "width": 50, "height": 20}},
                {"index": 4, "type": "link", "text": "Images", "bounds": {"x": 660, "y": 50, "width": 60, "height": 20}},
            ]
    ...
    def _estimate_element_bounds(self, index: int, total_elements: int, image_size: Tuple[int, int]) -> Dict:
    ...
    def _map_to_browser_index(self, element_number: int, element_map: Dict) -> Optional[int]:
