...
class CUDAGPUManager:
    ...
    def _check_cuda_availability(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    "PyTorch not available, checking llama-cpp-python CUDA support"
                )
            except Exception as e:
                logger.warning(
                    f"PyTorch CUDA check failed: {e}, trying llama-cpp-python"
                )
            ...
            try:
                ...
                if "n_gpu_layers" not in init_signature.parameters:
                    logger.warning(
                        "llama-cpp-python compiled without CUDA support (no n_gpu_layers parameter)"
                    )
                    return False
            except Exception:
                pass
    ...
    def get_memory_info(self, device: int = None) -> GPUMemoryInfo:
        ...
        try:
            ...
            logger.debug(
                f"PyTorch GPU memory info failed: {e}, trying nvidia-smi fallback"
            )
                ...
                if result.returncode == 0:
                    ...
                    if lines and device < len(lines):
                        ...
                        if len(memory_data) == 3:
                            ...
                            logger.info(
                                f"GPU memory via nvidia-smi: {free_gb:.1f}GB free / {total_gb:.1f}GB total"
                            )
            ...
            if self.cuda_available:
                logger.warning(
                    "Using default GPU memory estimates (8GB total, 6GB free)"
                )
                return GPUMemoryInfo(
    ...
    def optimize_gpu_layers(
        self, total_layers: int, model_size_gb: float, model_path: str = None
    ) -> int:
        """
    ...
    def profile_model_loading(
        self, model_path: str, actual_size: float, load_time: float, gpu_layers: int
    ) -> ModelMemoryProfile:
        """
    ...
    def start_monitoring(self):
        ...
        logger.info(
            f"GPU Manager start_monitoring called. Has config: {hasattr(self, 'config') and self.config is not None}"
        )
        ...
        if hasattr(self, "config") and self.config:
            ...
            if hasattr(self.config, "gpu"):
                logger.info(
                    f"Config has gpu section. Has enable_monitoring: {hasattr(self.config.gpu, 'enable_monitoring')}"
                )
                if hasattr(self.config.gpu, "enable_monitoring"):
                    logger.info(
                        f"gpu.enable_monitoring value: {self.config.gpu.enable_monitoring}"
                    )
            ...
            if hasattr(self.config, "monitoring"):
                logger.info(
                    f"Config has monitoring section. Has gpu_monitoring: {hasattr(self.config.monitoring, 'gpu_monitoring')}"
                )
                if hasattr(self.config.monitoring, "gpu_monitoring"):
                    logger.info(
                        f"monitoring.gpu_monitoring value: {self.config.monitoring.gpu_monitoring}"
                    )
        ...
        while self._monitoring_active:
            try:
                ...
                with self._lock:
                    ...
                    self.memory_history = [
                        info
                        for info in self.memory_history
                ...
                if memory_info.utilization > 0.95:
                    ...
                    logger.warning(
                        f"GPU overloaded: {memory_info.utilization:.1%} usage"
                    )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            if self.memory_history:
                avg_utilization = sum(
                    info.utilization for info in self.memory_history
        ...
        return {
            ...
            "models": {
                ...
                "total_estimated_size": sum(
                    p.estimated_size for p in self.model_profiles.values()
                ),
                "average_performance": sum(
                    p.performance_score for p in self.model_profiles.values()
                )
                / max(len(self.model_profiles), 1),
            },
